...
export default function EditProductPage() {
    ...
    const handleCreateOption = async () => {
        ...
        try {
            const valuesArray = newOption.valuesStr.split(',')
                .map(v => v.trim()).filter(v => v !== '').map(v => ({ value: v }));
    ...
    const handleDeleteOptionValue = async (valueId) => {
        if (!window.confirm("Xác nhận xóa/ẩn giá trị này?")) return;
        try {
    ...
    const handleSaveSkusMatrix = async () => {
        ...
        try {
            const payload = { ...product, skus: skus };
            await updateProduct(id, payload);
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto font-sans text-md pb-20">
            ...
            <div className="grid grid-cols-1 gap-6">
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                    <h2 className="text-xl font-bold mb-4 border-b pb-2">Thông tin cơ bản</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            <label className="block font-medium mb-1">Giá bán hiển thị (Base Price VNĐ)</label>
                            <input
                        ...
                        <div className="md:col-span-2">
                            <label className="block font-medium mb-1">Ảnh đại diện chung</label>
                            <div className="w-48">
                                <ImageUpload
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                    <h2 className="text-xl font-bold mb-4 border-b pb-2">Quản lý Thuộc tính (Màu sắc, Kích cỡ...)</h2>
                        ...
                        {options.map((opt) => (
                            <div key={opt.id} className="p-4 border border-gray-200 rounded-md bg-gray-50/50">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    <button onClick={() => handleDeleteOption(opt.id)} className="text-red-500 hover:text-red-700 text-sm font-medium flex items-center gap-1 cursor-pointer">
                                ...
                                <div className="flex flex-wrap gap-2 items-center">
                                    {opt.values?.map(val =>
                                        val.isActive !== false && (
                                            <span key={val.id} className={`px-3 py-1.5 border rounded-md text-md flex items-center gap-2 shadow-sm ${val.isActive !== false ? 'bg-white border-gray-300' : 'bg-gray-200 border-gray-300 text-gray-500 line-through'}`}>
                                                {val.value}
                                    ...
                                    <div className="ml-2 relative">
                                        <input
                                            ...
                                            className="border border-dashed border-gray-400 px-3 py-1.5 text-md rounded-md w-40 focus:outline-none focus:border-blue-500 focus:bg-white bg-transparent transition-all"
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    e.preventDefault();
                    ...
                    <div className="p-4 border border-blue-100 rounded-md bg-blue-50/30">
                        <h3 className="text-md font-bold mb-3 text-blue-800 flex items-center gap-2">
                        ...
                        <div className="flex flex-col md:flex-row gap-3">
                            ...
                            >
                                {isProcessingOption ? 'ĐANG XỬ LÝ...' : 'THÊM'}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 border-t-4 border-t-blue-600">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-6 border-b pb-4 gap-4">
                        <div>
                            ...
                            <p className="text-gray-500 mt-1">Cài đặt ảnh, mã tra cứu và tồn kho cho từng phân loại. Giá nhập/bán được quản lý riêng ở module Pricing.</p>
                        </div>
                        <button
                            onClick={handleSaveSkusMatrix}
                            disabled={isSavingSkus}
                            className="cursor-pointer bg-blue-600 text-white px-6 py-3 rounded-md text-md font-bold hover:bg-blue-700 flex items-center gap-2 shadow-lg shadow-blue-200 transition-all disabled:opacity-50"
                        >
                    ...
                    {skus && skus.length > 0 ? (
                        <div className="overflow-x-auto rounded-lg border border-gray-200">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-100/80 border-b border-gray-200 text-sm uppercase tracking-wider text-gray-600">
                                    ...
                                    {skus.map((sku, index) => {
                                        ...
                                        return (
                                            <tr key={sku.id || index} className={`border-b border-gray-100 hover:bg-gray-50/50 transition-colors ${isHidden ? 'bg-gray-50 opacity-60' : ''}`}>
                                                ...
                                                <td className="p-3">
                                                    <input
                                                        type="text" className="border border-gray-300 p-2 rounded w-full focus:ring-1 focus:ring-blue-500 outline-none disabled:bg-gray-100 uppercase"
                        ...
                        <div className="text-center py-10 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
                            <Plus className="text-gray-400 mx-auto mb-3" size={24} />
                            Sản phẩm chưa có biến thể SKU nào.
                        </div>
                    )}
                </div>
            </div>
